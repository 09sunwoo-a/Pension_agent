...
# 호출 직원 식별자 — 에이전트의 감사 기록·쿼터 버킷. 에이전트는 이 값이 **사번 7자리로
# 시작하면** 그것을 쪽지의 받는 사람·보내는 사람으로 쓴다(뒤에 접미가 붙어도 구분자로
# 이었으면 읽는다 — `3902172-<uuid>`).
#
# 지금 값은 **개발자 사번**이다. 쪽지가 이 사번 앞으로 가고 행내 기록도 이 사번으로
# 남는다 — 다른 사람이 이 클라이언트로 시험하면 자기 사번으로 바꾼다. 운영에서는
# 로그인한 직원의 사번이 여기 실려 온다(아직 한참 뒤의 일이다).
X_CLIENT_USER = "3902172"
