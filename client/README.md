...
| `employee_id` | 아니오 | 로그인한 직원의 **WorkB 사번**. 쪽지의 기본 수신자이자 발송 주체이고, 상담이력에 «누가 상담했나»로 남는다. 없으면 `x_client_user` 를 쓰되 **사번 꼴(숫자 7자리)일 때만** 읽는다 — 그 값은 쿼터 버킷 이름이라 사번이라는 보장이 없기 때문이다. 둘 다 사번이 아니면 서버의 `WORKB_EMP_NO` 로 떨어지고, 그것도 없으면 에이전트가 쪽지 발송을 제안하지 않는다 |

- 고객을 질문 문장에서 알아내는 기능은 없다. 어느 고객인지는 **호출자가 정한다.**
- `x_client_user` 가 사번 그대로면 `employee_id` 를 따로 보낼 필요가 없다. **다른 값이면
  보낸다** — 안 보내면 쪽지가 서버 설정의 사번 한 명 앞으로 몰려 나간다.
