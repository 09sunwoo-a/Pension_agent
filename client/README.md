...
| `x_client_user` | 예 | 호출한 직원 식별자. **「사번 7자리 + uuid」** 꼴로 보낸다(`3902172-550e8400-…` — uuid 는 LLM 중복 호출을 가르는 값이다). 감사 기록이자 LLM 쿼터 버킷이고, 에이전트는 여기서 **앞 7자리를 사번으로** 읽어 쪽지의 받는 사람·보내는 사람을 정한다 |
