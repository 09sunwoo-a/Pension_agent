...
| `action` | `kind`, `label`, `prompt` (+ 쪽지면 `title`, `text`, `to`, 있을 때만 `note`, `options[]`) | 연계 제안 턴에만, `answer` 다음. **답변 본문 바로 아래**에 `prompt` 와 **네 / 아니오 버튼**을 그린다(아래 「제안은 근거가 아니다」). 누르면 다음 턴 `message` 로 `"네"` 또는 `"아니오"` 를 보낸다. `options` 가 있으면 네 / 아니오 **대신** 그 버튼들을 그린다(아래 「쪽지 초안의 `note` 와 `options`」) |
...
`answer.text` 의 마지막 줄도 같은 문장(`— {prompt}`)이다(쪽지의 `note`·`options` 가 있는 턴은
그 앞뒤에 줄이 더 붙는다 — 아래 절). 버튼을 따로 그리는 프론트는 본문 끝의 그 줄을 떼고 그
자리에 세운다 — 같은 문장이 두 번 서지 않게 하되, **본문과 버튼 사이에 출처·근거 블록이
끼어들지 않게** 한다.

### 쪽지 초안의 `note` 와 `options`

쪽지(`kind: "memo"`) 제안에는 필드가 둘 더 올 수 있다. **없으면 키 자체가 없다.**

| 필드 | 언제 오나 | 화면 |
|---|---|---|
| `note` | 직원이 초안 수정을 지시했는데, 에이전트가 지시 밖에서도 함께 고친 것이 있을 때. 예: `"함께 고친 것: 호칭을 받는 사람에 맞춤"` | 쪽지 초안 카드 **아래**, `prompt` 위에 **작은 회색 글씨**로 한 줄. 제목처럼 크게·굵게 그리지 않는다 — 직원에게 묻는 말이 아니라 초안에 딸린 부기다 |
| `options` | 이름으로 보내려는데 같은 이름의 직원이 여러 명일 때. 예: `["WM플랫폼부(P) 대리 · 사번 3901317", "대출실행센터 선임팀장 · 사번 2768578"]`. 이때 `prompt` 는 `"어느 분께 보낼까요?"` | `prompt` 아래에 **네 / 아니오 버튼을 그리지 않고** `options` 의 항목마다 버튼을 하나씩 그린다. 누르면 그 항목 문자열을 그대로 다음 턴 `message` 로 보낸다(안의 사번으로 받는 사람이 정해진다). 직원이 「1번」·「WM」처럼 직접 입력해도 된다 |

**화면 순서는 둘 다 같다: 에이전트가 하는 말 → 쪽지 초안 카드 → `note` → `prompt` → 버튼.**

`answer.text` 에도 같은 내용이 들어 있다(버튼을 그리지 않는 화면을 위한 것). 버튼을 그리는
프론트는 본문에서 아래 부분을 떼고 그 자리에 위 순서대로 세운다.

- `note` 가 있으면 본문에서 `note` 문장과 **같은 줄**부터 끝까지를 뗀다.
- 없으면 `— {prompt}` 로 시작하는 줄부터 끝까지를 뗀다. `options` 가 있는 턴은 그 줄 아래에
  `1. …` 번호 목록이 이어지는데, 그것까지 함께 떼진다.

```json
{"type": "answer", "text": "이선우 님이 3명 있어요.\n\n— 어느 분께 보낼까요?\n1. WM플랫폼부(P) 대리 · 사번 3901317\n2. 대출실행센터 선임팀장 · 사번 2768578\n3. 재무기획부 수석차장 · 사번 2827375", "intent": null, "links": []}
{"type": "action", "kind": "memo", "label": "이 쪽지 보내기(받는 사람: 이선우 님)", "prompt": "어느 분께 보낼까요?",
 "title": "…", "text": "…", "to": "이선우 님",
 "options": ["WM플랫폼부(P) 대리 · 사번 3901317", "대출실행센터 선임팀장 · 사번 2768578", "재무기획부 수석차장 · 사번 2827375"]}
```

```json
{"type": "answer", "text": "<초안 코드블록>\n\n함께 고친 것: 호칭을 받는 사람에 맞춤\n\n— 이대로 쪽지를 보낼까요? 받는 사람은 이선우 님이에요. (네 / 아니오)", "intent": null, "links": []}
{"type": "action", "kind": "memo", "label": "…", "prompt": "이대로 쪽지를 보낼까요? 받는 사람은 이선우 님이에요. (네 / 아니오)",
 "title": "…", "text": "…", "to": "이선우 님", "note": "함께 고친 것: 호칭을 받는 사람에 맞춤"}
```
