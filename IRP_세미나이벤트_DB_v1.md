...
> **일정 이동:** 시연일보다 먼저 끝나는 일정이 생겨 2026-09-16 에 9건 전부 8주(56일) 뒤로 옮겼다(요일은 그대로). EVT-004 종료일만 해를 넘기지 않게 2026-12-30 으로 잡았다 — 안내 문구가 월/일만 적어 「1/8까지」가 지난 1월로 읽힌다. `src/pension_agent/strategy_agent/data/assets.json` · `outreach_golden.json` 이 같은 값을 갖는다
...
- `golden_dataset`:
  ...
  > 11/3(화) 17시 'IRP 수령 방법' 온라인 세미나가 열려요.  
...
- `golden_dataset`:
  > (광고) [고객명] 고객님, KB국민은행입니다.  
  > 11/5(목) 16시 '퇴직연금 디폴트옵션 점검' 온라인 세미나를 안내드려요.  
...
- `golden_dataset`:
  > (광고) [고객명] 고객님, KB국민은행입니다.  
  > 11/10(화) 17시 '퇴직연금 자산배분 전략' 온라인 세미나가 열려요.  
...
- `golden_dataset`:
  > (광고) [고객명] 고객님, KB국민은행입니다.  
  > 11/12(목) 17시 '내 투자성향에 맞는 퇴직연금 포트폴리오 찾기' 세미나를 안내드려요.  
...
- `golden_dataset`:
  > (광고) [고객명] 고객님, KB국민은행입니다.  
  > 11/17(화) 17시 '금리 변화기 퇴직연금 운용 전략' 세미나를 안내드려요.  
...
- `organizer`: `KB스타뱅킹`
- `schedule`: `2026-11-02 ~ 2026-11-25`
...
- `golden_dataset`:
  > (광고) [고객명] 고객님, KB국민은행입니다.  
  > 타 금융기관 IRP를 KB로 이전하는 고객 대상 이벤트를 12/11까지 진행해요.  
...
- `organizer`: `KB스타뱅킹`
- `schedule`: `2026-11-09 ~ 2026-12-25`
...
- `organizer`: `KB스타뱅킹`
- `schedule`: `2026-11-16 ~ 2026-12-30`
