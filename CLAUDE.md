...
**에이전트가 작업할 때 `06`~`09` 는 열지 않는다.** 원천 자료는 `01`~`05` 이고, 거기서
`scripts/kb_build` 가 뽑아낸 카드(`knowledge/data/kb_*.json`)가 실제로 쓰이는 형태다.
06~09 를 직접 읽는 것은 같은 내용을 두 번째 경로로 다시 들이는 일이다. 06 은 파일 하나가
130~430KB 라 한 번 열면 컨텍스트를 통째로 먹는다.

| 폴더 | 무엇 | 대신 볼 곳 |
|---|---|---|
| `06_주제별_추출지식` | 카드의 원문 (1.3MB · 5파일) | 생성된 카드 `knowledge/data/kb_*.json`, 저작 규약은 `src/AUTHORING.md` |
| `07_에이전트_기능정의` | 상위 요건 원문 | `docs/REQUIREMENTS.md` · `consult_agent/CLAUDE.md` |
| `08_인사이트` | 현장의 목소리 50건 | 생성된 `kb_fieldtips` 카드 |
| `09_코어비즈니스로직` | 설계 메모 — **어떤 코드도 읽지 않는다** | 구현과 `consult_agent/CLAUDE.md` 가 기준 |

**빌드는 계속 이 폴더들을 읽는다.** 위 규칙은 *에이전트의 읽기*에 대한 것이지 빌드 입력을
끊는 것이 아니다 — `scripts/kb_build/config.py` 는 01~08 을 전부 입력으로 잡는다.

예외는 사람이 명시적으로 지시할 때뿐이다. 원문을 고치는 작업(→ `build_kb` 재실행)이
대표적이다. 게이트로도 막고 싶으면 `.claude/settings.local.json` 에 `Read()` deny 를
둔다(gitignore 대상이라 각자 환경에서 켠다).

한 가지는 값을 치른다: 코드 주석 30여 곳이 `07/01 ① 필수 구성 요소 4` 같은 표기로 07 을
근거로 가리키는데, 그 원문을 열어 확인할 길이 막힌다. 임계값·요건의 출처를 확인해야 하는
상황이면 지어내지 말고 **사람에게 묻는다**.

## 무엇이 어디 있나

| | |
|---|---|
| 요건 기준 | `docs/REQUIREMENTS.md` — 전체 서비스 요건 앵커. 상위 기준 원문은 `07_에이전트_기능정의/` 지만 열지 않는다(→ «읽지 않는 폴더») |
