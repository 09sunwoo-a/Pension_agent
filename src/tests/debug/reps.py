...
오늘로 센 값인가» 하나였다). 지금은 **회귀 테스트 모듈이 고정을 스스로 건다**
(`tests.pin_today`) — 임포트로는 아무것도 걸리지 않는다. 그리고 **실행 머리에 시간축을
찍는다**(`_print_clock`): 러너가 제 시계를 읽지 않고 에이전트 산출의 출구가 주는 값을
그대로 옮긴다(`customer.ledger_stamp` · `clock.stamp`).
