...
FINAL: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    ...
    (3, "박정호 — 상담기억에서 업무 실행까지 (퇴직급여 1.5억 일반계좌 수령)", "168450-7293815", (
        ...
        # 있는지» 확인 항목으로만 두고 답이 없다. 60일·환급은 ⑧ 의 절차 카드(proc.041)에 남는다.
        ("⑦", "고객이 재취업 예정이래. 계좌는 어떻게 나눠 두는 게 좋아?"),
        ("⑧", "과세이연 등록은 어떻게 해?"),                        # proc.041 — 5단계 + 화면번호
        # 상담 마무리 — 이번 세션의 대화(transcript 도구)를 요약하고, 답변 뒤에 코드가 「쪽지로
        # 보낼까요?」를 붙인다(act._propose_memo). 승낙하면 본인 쪽지함으로 보낸다(연동 전 스텁).
...
def main(argv: list[str]) -> int:
    ...
    if final:
        ...
        print("                    M1 이 이번 세션 대화를 요약하고 «쪽지로 보낼까요?»가 붙는가 ·")
        print("                    M2 가 «쪽지를 보냈어요»로 끝나는가.")
    elif scenario:
