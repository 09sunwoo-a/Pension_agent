...
FINAL: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    ...
    (2, "이수민 — 상담 판단 및 화법 (예금 7,000만원 만기 + 현금성 1,000만원 대기)", "175926-3048171", (
        ...
        # 바꿨다(2026-09-03) — 디폴트옵션 수익률 순위(fact.k04.f62)는 브리핑에 수치가 없다.
        ("③", "고객이 국민은행의 디폴트옵션 수익률 얼마나 나오냐고 묻는데"),
...
def main(argv: list[str]) -> int:
    ...
    if final:
        ...
        print("                    ③ 이 디폴트옵션 수익률 순위를 기준시점과 함께 말하는가 ·")
        print("                    ⑤ 가 «이동하지 않는다»로 답하는가 · ⑦ 이 계좌 3단 분리로 답하는가 ·")
        print("                    M1 이 머리말(박정호 고객님·날짜) → ⑥⑦ 요약 항목 → [고객 주요 정보] 표로 나오고")
        print("                    «쪽지로 보낼까요?»가 붙는가 · «직접 처리할 수 없어요» 류 문장이 없는가 ·")
