...
**오늘은 실제 날짜다.** 리허설은 «상담 시점의 오늘»을 보는 자리이므로 고정하지 않는다.
얼려 보고 싶으면 `PENSION_TODAY=YYYY-MM-DD` 를 붙여 돌린다 — 밖에서 준 값은 그대로 산다.

2026-09-07 까지는 그렇지 않았다. 이 러너가 `tests` 패키지 안에 살아서 **임포트만으로**
회귀 테스트용 고정(원장 기준일 2026-08-24)을 물려받았고, 그건 회귀의 요건이지 리허설의
요건이 아니었다. 화면에 그 사실이 없으니 실제 날짜로 도는 줄 알고 돌린 사람이 답에 실린
「185일간 접촉 없음」을 지어낸 수치로 의심했다(값은 원장 그대로였고, 없던 것은 «어느
오늘로 센 값인가» 하나였다). 지금은 러너 진입에서 그 고정을 되돌리고(`tests.unpin_today`),
**실행 머리에 시간축을 찍는다**(`_print_clock`) — 러너가 제 시계를 읽지 않고 에이전트
산출의 출구가 주는 값을 그대로 옮긴다(`customer.ledger_stamp` · `clock.stamp`).

**`qa` 만 예외다.** 그 대본의 «기대»는 `scenarios.QA_TODAY`(2026-09-07) 기준으로 적혀
있어서 다른 날짜로 돌면 잔여일수·ISA 상태가 어긋난다. 어긋나면 머리에서 알린다 —
...
# **«오늘» 고정을 여기서 되돌린다 — `pension_agent` 를 임포트하기 전에.** 이 모듈은
# `tests` 패키지 안에 살아서 임포트만으로 회귀 테스트용 고정(원장 기준일)을 물려받는데,
# 그건 회귀의 요건이지 리허설의 요건이 아니다(`tests.unpin_today`). 아래 임포트가 돌면
# `customer` 가 PERSONAS 를 만들면서 잔여일수·경과일을 그때의 «오늘»로 굳히므로, 순서가
# 바뀌면 이 한 줄이 아무 일도 하지 않는다.
#
# **러너로 실행할 때만이다.** 회귀 테스트가 이 모듈을 임포트해 검사하므로
# (`tests/test_consult_agent.py`), 임포트만으로 되돌리면 그 스위트의 고정이 풀린다.
...
CHECKS: dict[str, tuple[str, ...]] = {
    "review": (
        ...
        "  ※ 이수민의 만기 요건은 2026-09-01 이후라야 선다 — 실제 날짜로 도니 지금은 그냥 선다."
        "  (옛날로 얼려 보려고 PENSION_TODAY 를 걸었다면 그 날짜를 확인한다.)",
    ),
    "library": (
        ...
        "  ※ 이수민(3번)의 만기 요건은 2026-09-01 이후라야 선다 — 실제 날짜로 도니 지금은 그냥 선다."
        "  (옛날로 얼려 보려고 PENSION_TODAY 를 걸었다면 그 날짜를 확인한다.)",
    ),
    "demo": (
    ...
    "qa": (
        ...
        "  ※ 기대는 2026-09-07 기준으로 적혀 있다(scenarios.QA_TODAY). 러너는 실제 날짜로 돌므로"
        "     잔여일수·ISA 상태가 그만큼 어긋난다 — 실행 머리에서 알린다.",
    ),
}
...
def main(argv: list[str]) -> int:
    ...
    # **어느 «오늘»로 도는지 먼저 찍는다.** 고정을 되돌리는 것(머리말)과 짝이다 — 되돌리기만
    # 하면 다음에 누가 `PENSION_TODAY` 를 걸어둔 채 돌릴 때 같은 혼동이 그대로 재현된다.
    # 러너가 제 시계를 읽지 않고 **에이전트 산출의 출구가 주는 값**을 옮긴다(clock.stamp 머리말).
