...
SCENARIOS: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    ...
    (3, "이수민 — 만기 임박 · 디폴트옵션 미등록 (골든 케이스 05)", "175926-3048171", (
        ...
        #
        # **되묻기 다음 턴이라 금액을 여기서 다시 읽지 않는다** — 「5,500만원」은 갈래 라벨이지
        # 납입액이 아니다. 그 구분이 없던 동안 이 턴이 1,485,000원(한도를 다 채운 값)을 답했다
        # (tools._extra_paid · 기준서 §5).
        ("L6", "5,500만원 이하야"),
    )),
...
def main(argv: list[str]) -> int:
    ...
    if final:
        ...
        print("                    L6 가 300만원 기준(495,000원)으로 답하는가 ·")
        print("                    S2 가 10개월 전 기록을 꺼내는가 ·")
