...
DEMO: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    ...
    (1, "1~2막 상담 전·중 — 송도윤(방치현금 54% · ISA 만기 · 322일 미접촉)",
     "188406-7352194", (
        ...
        ("T11", "이 고객한테 맞는 세미나·이벤트가 3건 열려 있다는데, 어느 걸 안내하면 좋을까?"),
...
SCENARIOS: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    ...
    (3, "이수민 — 만기 임박 · 디폴트옵션 미등록 (골든 케이스 05)", "175926-3048171", (
        ...
        ("L5", "300만원 더 넣으면 얼마 돌려받아?"),                  # 계산기
        # ↑ 이 고객은 잔여한도 900만원·당해 납입 0원이라 300만원이 그대로 공제 대상이다.
        # 총급여 구간이 원장에 없어 되묻기로 끝날 수 있는 턴이라(김서연 K3 와 같은 갈래)
        # **답이 나오는 턴을 반드시 뒤에 둔다** — 되묻기는 답을 미루는 것이 아니라 틀린 답을
        # 막는 것이므로(기준서 §5), 시연이 되묻기에서 끊기면 그 장치가 «답을 못 한 것»으로
        # 읽힌다. L6 는 되물었으면 그 답이고, 안 되물었으면 두 구간 중 하나로 좁히는 턴이다.
        ("L6", "5,500만원 이하야"),
    )),
    (4, "송도윤 — 복합 3종 · 10개월 전 기록이 명분 (기존 9케이스)", "188406-7352194", (
        ...
        ("S5", "이 고객한테 맞는 세미나·이벤트가 3건 열려 있다는데, 어느 걸 안내하면 좋을까?"),
...
FINAL: tuple[tuple[int, str, str | None, tuple[tuple[str, str], ...]], ...] = (
    (1, "김서연 — 제도 적용 (타행 ISA 8,000만원 만기 예정)", "171203-4815062", (
        ...
        # 실화면에서는 안내 콘텐츠 칩(suggest.outreach_chips — 김서연은 이벤트만 2건, 8/24·9/2
        # 모두 같다). 답변이 이벤트 이름을 인용해야 «발송 화면 열까요?»가 붙는다(act._propose_lms ③).
        ("E1", "이 고객한테 맞는 이벤트가 2건 열려 있다는데, 어느 걸 안내하면 좋을까?"),
