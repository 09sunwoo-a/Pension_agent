...
def check_screen_link() -> int:
    ...
    # 판정이 «직원이 물은 대상이 자료에 없다»(none)로 끝난 턴에는 제안을 붙이지 않는다 —
    # 「IRP 해마 등록 화면번호」에 이름 일부가 겹치는 화면을 안내한 답변에 그 화면을 열자는
    # 제안이 붙었다(2026-09-16 실측). 없다고 답한 턴의 곁가지 화면은 직원이 묻지 않은 것이다.
    hit = not act.offer({"answer": "그 화면은 자료에 없어요. 대신 [75-08-110] 화면이 있어요.",
                         "evidence": [proc], "customer_id": "TEST_ACT",
                         "judge_verdict": "none"}).get("pending_action")
    print(f"{'✓' if hit else '✗'} «물은 대상이 자료에 없다»로 판정된 턴에는 연계를 제안하지 않는다")
    ok += hit
