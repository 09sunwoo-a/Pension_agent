...
from pension_agent.strategy_agent.customer import PERSONAS, Profile, conditions

from pension_agent.strategy_agent import agent

BY_NAME = {p.nm: p for p in PERSONAS}
# 선정 **로직**을 검사하는 픽스처는 노출 개수를 넓게 열어 만든다.
# 화면 노출은 "제안 1개 + 예비 1개"로 좁혔지만(engine.TOP_N/ALT_N, 07_에이전트_기능정의/01 ① 4),
# 이 파일의 대부분은 "이 고객에게 어떤 전략이 성립하는가"를 보는 것이라 노출 개수와 무관해야
# 한다. 노출 개수 자체는 아래 「제안 개수」 절에서 기본값으로 따로 검사한다.
_WIDE = 99
...
for nm, f in FACTS.items():
    ...
    p = BY_NAME[nm]
    check(bf.get("source") == "proc.001", f"{nm}: briefing 근거 proc.001")
...
# 고객 헤더 — 최근 접촉·스타클럽 등급(07_에이전트_기능정의/01 ① 양식 "만 57세 · VIP",
...
if engine.DOC_TITLES:
    ...
    # 산출물 출처에 카드 id 앞토막(코드용 이름)이 새어나오지 않는다.
    check(all(not s.startswith(("pitch", "seg", "proc", "m.", "fact", "kb_"))
              for s in _lee["source_titles"]),
          "산출물 출처에 코드용 id 미노출", str(_lee["source_titles"]))
...
# ─────────────────────────────────────────────────────────────
# 소스 미확정 필드 — 값이 안 와도 브리핑이 죽지 않는다
#
# retPct(피어그룹 수익률 백분위)·dorm(최근 접촉 경과일)은 데이터딕셔너리에 소스가 없어
# 실데이터 전환 초기에 비어 올 수 있다(REQUIREMENTS.md §4). 이전 구현은 None 을 정수와
# 비교해 TypeError 로 브리핑 전체를 죽였다.
# ─────────────────────────────────────────────────────────────

from dataclasses import replace as _replace

_base = BY_NAME["김민수"]
for _fld in ("retPct", "dorm"):
    try:
        _f = engine.prepare(_replace(_base, **{_fld: None}))
        check(True, f"{_fld}=None 이어도 브리핑 생성")
        check(_fld != "retPct" or "low" not in [c.split(":")[0] for c in _f["conditions"]],
              "retPct=None 이면 low 요건 미성립")
        check(_fld != "dorm" or "dor" not in [c.split(":")[0] for c in _f["conditions"]],
              "dorm=None 이면 dor 요건 미성립")
        check("None" not in str(_f["customer"]["수익률"]) and
              "None" not in str(_f["customer"]["최근접촉"]),
              f"{_fld}=None 이 화면 문자열로 새지 않음",
              f"{_f['customer']['수익률']} / {_f['customer']['최근접촉']}")
    except Exception as _e:  # noqa: BLE001
        check(False, f"{_fld}=None 이어도 브리핑 생성", f"{type(_e).__name__}: {_e}")

check(engine.prepare(_replace(_base, retPct=None, dorm=None))["items"] is not None,
      "retPct·dorm 둘 다 None 이어도 제안 산출")


# ─────────────────────────────────────────────────────────────
# 「고객이 모르는 자기 현황」 재료 — 대화형이 답할 수 있어야 한다
#
# 07_에이전트_기능정의/01 ① 필수 구성 요소 3. 화면 칸이 아니라 재료로 싣는다(대화형 몫).
# 재료에 없으면 consult_agent 의 답을 verify() 가 "재료 밖 수치"로 거부한다.
# ─────────────────────────────────────────────────────────────

_bq = engine.prepare(_base)["briefing"]
check("당해_납입액" in _bq and "예상_세액공제액" in _bq,
      "briefing 재료에 납입액·공제액 존재", str(sorted(_bq))[:80])
check(_bq["당해_납입액"] == engine.won(_base.pension_paid_ytd),
      "당해 납입액이 Profile 값과 일치", _bq["당해_납입액"])
check(engine.verify(f"올해 납입액은 {_bq['당해_납입액']}입니다.", engine.prepare(_base))[0],
      "납입액 답변이 verify() 를 통과")
check(not engine.verify("올해 납입액은 700만원입니다.", engine.prepare(_base))[0],
      "재료 밖 납입액은 여전히 거부")
...
# ─────────────────────────────────────────────────────────────
# 제안 개수 — "오늘의 제안 1개 (+예비 1개)"
#
# 07_에이전트_기능정의/01 ① 필수 구성 요소 4: "우선순위가 정해져서 옴. 제안 5개 나열은
# 안 하느니만 못함." 위 FACTS 는 로직 검사용으로 개수를 넓게 열어 만들었으므로, 실제
# 노출 개수는 여기서 기본값으로 따로 본다.
# ─────────────────────────────────────────────────────────────
...
for _p in PERSONAS:
    _d = engine.prepare(_p)
    check(len(_d["items"]) <= 1, f"{_p.nm}: 제안 최대 1건", str(len(_d["items"])))
    check(len(_d["alternatives"]) <= 1, f"{_p.nm}: 예비 최대 1건", str(len(_d["alternatives"])))
    # 좁혀도 평가 자체는 전부 한다 — 무엇이 성립했는지는 candidates 에 남는다.
    _wide = engine.prepare(_p, top_n=_WIDE)
    check(len(_d["candidates"]) == len(_wide["candidates"]),
          f"{_p.nm}: 노출을 좁혀도 후보 평가는 동일",
          f"{len(_d['candidates'])} vs {len(_wide['candidates'])}")
    if _d["items"]:
        check(_d["items"][0]["id"] == _d["candidates"][0]["id"],
              f"{_p.nm}: 노출 제안은 최고점 후보")
    check({a["id"] for a in _d["alternatives"]}.isdisjoint({i["id"] for i in _d["items"]}),
          f"{_p.nm}: 예비와 제안이 겹치지 않음")
