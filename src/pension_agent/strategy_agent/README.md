...
산출 예시 — 시연용 목업 9케이스의 '이준호'(만기도래, KB-PIN 198734-1205842):

```
보유현황  예금 36% · TDF·MP 39% · 섹터ETF 25% · 수익률 13.7% · 만기 D-17 4,050만원

[전략 제안] 만기 D-17 예금 4,050만원을 KB GIC 확정금리(연 3.65%)로 예치하도록 제안하세요.
[판단근거] · 만기 D-17 도래 — 재예치 지시가 없으면 현금성자산(연 2.52%)으로 대기하므로
             만기 전 운용 방향 확정이 필요
[근거 문서] 개인형IRP 고객관리 가이드 「IRP야, KB를 떠나지 마오!」 [Series 1] IRP 수익률 관리
```
...
# 산출
python -m pension_agent.strategy_agent.agent 이준호  # 단일 고객 전략 제안 (CLI)
...
**같은 입력이면 한 번만 만든다.** 이 함수는 LLM 으로 산문을 쓰므로 부를 때마다 다른 문장이
나오는데, 산출을 읽는 곳이 둘이다 — 브리핑 화면과, 그 화면을 보며 묻는 직원에게 답하는
대화형(`consult_agent` 의 `customer` 도구가 `sentence`·`insight` 를 재료에 그대로 싣는다).
각자 생성하면 화면에 뜬 문장과 대화가 인용하는 문장이 갈린다. 캐시 키는 **프로파일 내용**
이고(같은 id 에서 파생시킨 다른 프로파일이 서로의 결과를 받지 않는다), 반환은 복사본이라
호출부가 고쳐도 다음 호출자가 오염된 브리핑을 받지 않는다. 입력이 바뀌면(고객 원장 재적재,
테스트의 LLM 스텁 교체) `agent.clear_briefing_cache()` 로 비운다.

이 공유는 **프로세스 안에서만** 성립한다. 무한히 쌓이지 않도록 상한(`_BRIEFING_MAX`)에서
