...
# 연금저축 **단독** 세액공제 한도(원). 위 900만원은 IRP 를 포함했을 때의 한도이고, 연금저축
# 상품(보험·펀드·신탁)에만 납입하면 600만원까지만 공제된다. 출처: fact.k04.f3 (kb_facts)
# 「연금저축 단독 600만원 vs IRP 포함 900만원」.
#
# 이 값은 **어디에 넣느냐가 답을 가르는 지점**을 재는 데만 쓴다(consult_agent/tools.py
# `_tax_credit`). 연금저축 기준의 환급액을 계산하지는 않는다 — 원장의 세액공제 잔여한도는
# 연금저축·IRP 를 합산한 값 하나뿐이라, 그중 연금저축에서 이미 쓴 몫을 알 수 없다.
PENSION_SAVINGS_CAP_WON = 6_000_000
