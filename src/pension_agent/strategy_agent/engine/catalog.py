...
# 예금 잔액을 물었을 때 실제보다 큰 값을 읽는다.
#
# 라벨이 «예금·현금성» 이던 동안 정확히 그 오독이 났다: 고유계정대 75% + 예금 15% 인
# 고객이 "예금·현금성 90%" 로 나가서 직원이 예금을 90% 로 읽었고, 실제 예금은 15% 였다.
# 앞머리가 "예금" 이면 뒤에 무엇을 붙여도 예금 비중으로 읽힌다. 룰베이스의 «원리금보장_
# 합산비율»(고유계정대·정기예금·원리금보장상품 합산, TG-202)이 바로 이 값이라 그 이름을 쓴다.
#
# 3분류 뷰(render._three_way)의 «원리금보장형» 과는 다른 값이다 — 그쪽은 고유계정대를
# **빼고** 세는 예금+GIC+기타다. 그래서 «원리금보장형» 이 아니라 «원리금보장·현금성» 이다.
PORT_LABELS = ("원리금보장·현금성", "채권형", "TDF·MP", "섹터ETF")
