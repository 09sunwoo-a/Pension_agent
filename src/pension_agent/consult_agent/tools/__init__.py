...
TOOLS: dict[str, Tool] = {
    t.name: t for t in (
        ...
        Tool("last_answer", "**이 에이전트가 방금 한 답변**의 원문 — 직원이 그 답변을 «더 "
             "짧게»·«더 쉽게»·«고객 대사만»·«핵심만»·«다시 써줘»처럼 **고쳐 써 달라고** 할 때 "
             "쓴다. 새 검색이 아니라 있던 답을 다시 쓰는 재료라 대개 이것 하나로 끝난다. "
             "직전 답변에 없던 내용을 더해 달라면(«수수료도 넣어서») 그 재료의 도구를 함께 "
             "부른다. 화면(AI브리핑)의 문장을 고치는 요청은 여기가 아니다",
             _last_answer, progress="직전 답변"),
    )
}
