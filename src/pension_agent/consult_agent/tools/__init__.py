...
TOOLS: dict[str, Tool] = {
    t.name: t for t in (
        ...
        # 설명이 곧 계획의 판단 재료다 — 여기 없는 항목은 계획이 이 도구로 오지 않는다.
        # 「최근 한 달 사이에 고유계정대가 얼마나 늘었어?」(질문 리스트 13번)의 답은 이 재료의
        # 거래활동 줄(최근 1개월 고유계정대 증감 1억 7,000만원)에 있었는데, 설명이 잔액·수익률·
        # 요건만 말해 계획이 시황·제도 수치·방법론을 돌고 «자료 없음»으로 끝났다(2026-09-21
        # 행내 실측). 재료에 실린 축을 설명에도 적는다.
        Tool("customer", "지금 열려 있는 고객의 브리핑 자료(잔액·수익률·보유상품·거래활동(최근 "
             "매매·입금·최근 1개월 고유계정대 증감)·계좌 상태·성립 요건, 그리고 이 고객이 "
