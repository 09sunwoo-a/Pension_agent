...
def outreach_chips(customer_id: str | None) -> list[str]:
    ...
    **관련도 0 인 폴백은 세지 않는다.** 화면 ⑨ 는 섹션을 비우지 않으려고 이 고객과 무관해도
    ...
    # 건수는 «있다는데»로 싣는다 — 칩은 직원이 누르는 순간 직원의 질문이 되는데, 「2건
    # 있어.」는 직원이 알 수 없는 사실을 직원이 단정하는 꼴이라 어색하다. 상담 기록 칩이
    # 날짜·경과일을 괄호로 싣는 것과 같은 자세다: 알림은 실리되 문장은 질문으로 읽힌다.
    return [f"이 고객한테 맞는 {'·'.join(kinds)}가 {len(rows)}건 열려 있다는데, "
            "어느 걸 안내하면 좋을까?"]
