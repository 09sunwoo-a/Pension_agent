...
#: 화면에서 쪽지 초안을 감싸는 코드블록 펜스(effects/memo · nodes/act.offer). 초안의 일부가
#: 아니라 **화면 장치**다 — 나가는 본문에는 없고, 세션 기록에서 재료를 만들 때는 뗀다
#: (tools/history._strip_devices). 감싸는 이유는 «여기까지가 쪽지»를 직원이 화면에서 가릴 수
#: 있어야 하기 때문이다. 쓰는 쪽(effects)과 떼는 쪽(tools)이 서로를 임포트하지 않도록 답변
#: 기록 포맷의 소유자인 여기에 둔다.
FENCE = "```"
