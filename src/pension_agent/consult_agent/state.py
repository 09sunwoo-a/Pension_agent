...
class AgentState(TypedDict, total=False):
    ...
    # 부른 도구가 **죽은** 기록 — {"tool": 이름, "reason": 원인}. 빗나간 호출(plan_misses)과
    # 갈라 두는 이유는 셋 다 다르기 때문이다: 계획에게는 «말을 바꿔라»가 아니라 «이 도구는
    # 이번 턴에 없다»이고(tools.usable), 원장이 빈 채 끝난 턴의 답은 «자료가 없습니다»가
    # 아니라 «읽지 못했습니다»이며(nodes/plan.py::compose), 계측은 그 턴을 정상 턴으로
    # 세면 안 된다(graph.ask 의 turn_outcome). 하나로 접으면 셋이 함께 틀린다.
    plan_failed: list[dict]
