...
#
# ━━ «어디에 넣는 계산인가»를 재료가 말한다 ━━
# "300만원 더 넣으면 얼마 돌려받아?" 의 300만원은 **어느 계좌에 넣는 돈인지가 질문에 없다.**
# 계산은 이 고객의 개인형IRP 에 넣는 것으로 하는데(원장의 세액공제 잔여한도가 그 계좌의
# 여력이다), 재료가 그 전제를 적지 않으면 답변도 적지 않는다 — 직원은 어느 계좌 기준의
# 금액인지 모른 채 고객에게 옮긴다. 전제는 §5 가 되묻기 대신 요구하는 것이기도 하다
# ("맥락으로 추측이 서면 전제를 밝히고 답한다").
#
# **다만 전제로 덮을 수 없는 금액대가 있다.** 연금저축에만 납입하면 단독 한도가 600만원이라
# (fact.k04.f3), 추가 납입액이 그 한도를 넘으면 IRP 냐 연금저축이냐가 **환급액을 실제로
# 가른다.** 그때는 갈래를 재료에 실어 되묻기 판정이 보게 한다(§5) — 전제 한 줄로 덮고
# 넘어가면 조용히 다른 계좌 기준의 금액을 답하게 된다.
# ─────────────────────────────────────────────────────────────

#: 공제율의 근거 카드. 세율·한도·아래 단서가 전부 여기서 온다.
TAX_FACT_ID = "fact.k04.f2"

#: 연금저축 단독 한도의 근거 카드. 계좌 갈래가 답을 가를 때만 재료에 실린다.
SAVINGS_CAP_FACT_ID = "fact.k04.f3"
...
def _tax_credit(state: AgentState, query: str) -> Evidence | None:
    ...
    lines = [f"■ 세액공제 환급 예상액 — {p.nm} 고객 (시스템 계산 — 검색 결과가 아니다)",
             f"· 어디에 넣는 계산인가: 이 고객의 **개인형IRP 계좌에 추가로 납입하는 경우**다",
             f"· 당해 납입액 {_won(paid)} · 세액공제 한도 {_won(cap)} · 잔여한도 {_won(room)} "
             f"— 이 한도와 잔여한도는 연금저축 납입분까지 합산한 값이다(원장 값)",
    ...
    if gain <= 0:
        ...
        # 계좌 갈래가 답을 가르는 금액대에서만 싣는다. 아래이면 IRP·연금저축 어디에 넣어도
        # 이 금액이 그대로 공제 대상이라, 갈래를 세우는 것은 무관한 경고를 하나 더 다는
        # 일이다(CLAUDE.md §7 — 무관한 경고가 늘수록 관계있는 경고가 묻힌다).
        savings = KB.facts.get(SAVINGS_CAP_FACT_ID)
        if savings is not None and gain_base > CUST.PENSION_SAVINGS_CAP_WON:
            lines.append(
                f"· **연금저축에 넣는 경우라면 위 금액이 아니다** — 연금저축 단독 세액공제 "
                f"한도는 {_won(CUST.PENSION_SAVINGS_CAP_WON)}이라 "
                f"{_won(gain_base)}을 다 공제받지 못한다. 위 계산은 개인형IRP 기준이고, "
                f"어느 계좌에 넣는지에 따라 환급액이 갈린다")
            cards.append(savings)
