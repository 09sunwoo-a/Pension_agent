...
def _date(state: AgentState, query: str) -> Evidence | None:
    ...
    #
    # 남는 구멍 하나를 적어 둔다. `verify._measures` 는 "2026년 9월"을 «연월 한 덩이»로
    # 끊으면서 허용 형태에 **연·월 토큰을 따로** 담는다. 그래서 원장 어딘가에 2026 이
    # 있으면 2026년의 아무 달이나 통과한다 — 틀린 날짜를 수치 집합 검사로는 못 잡는다는
    # 뜻이다. atomic 으로 막으려 해 봤자 스팬의 연도(2026)가 "올해(2026년)" 같은 무관한
    # 문장과 겹쳐 **답변 전체가 폐기**되므로 더 나쁘다. 지금 이 자리를 지키는 것은
    # ANSWER_SHAPES["date"](남은 일수만 쓰고 기준 날짜를 빼지 않는다)와 작성 규약 1번
    # (재료 밖 날짜 계산 금지)이다. 제대로 막으려면 verify 쪽에서 날짜를 통짜로 대조해야
    # 하는데, 그건 이 도구가 아니라 검증기의 일이다(같은 부류: allow 를 넓혀 오답 날짜가
    # 통과했던 _POOL_KEYS 주석).
