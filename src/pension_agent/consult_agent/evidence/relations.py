...
한해** 대신한다 — 선언이 없으면 `atomic` 이 그대로 남는다(tools/base.py). 저작이 넓어지는 만큼
원문 강제가 물러난다.
...
따옴표 없이 정정으로 인정하는 꼴은 하나뿐이다 — 문구 **바로 뒤**에 부정이 붙은 것
(「전환금 전액이 공제 대상이 되는 것이 아니에요」, `_corrects` ②). 그건 인용이 아니라 반대
주장이다.
...
#:
#: **「오답」·「오해」가 빠져 있던 것은 데이터와 어긋난 자리였다.** 카드의 `verify_points` 는
#: 그 표현을 그대로 쓴다(F47 — «"이미 통장으로 받았으면 끝" = 오답(60일 내 재입금 가능)»)
#: 그 줄은 작성 프롬프트에 실려 «직원에게 이렇게 짚어주라»고 시키는데, 시킨 대로 쓴 답변이
#: 정정 표지 목록에 그 낱말이 없다는 이유로 폐기됐다 — 이 파일 머리말이 적어 둔 「데이터가
#: 시킨 일을 했다고 벌하는 것」의 두 번째 사례다(2026-09-02 실측: 「퇴직금을 이미 통장으로
#: 받았는데 IRP 로 되돌릴 수 있어?」가 매번 근거 원문 덤프로 끝났다).
CORRECTIONS = ("오기", "오타", "표기 오류", "틀린", "틀립", "틀려", "잘못", "오답", "오해",
...
#: 오답 문구 **바로 뒤**에 붙은 부정 — 「전환금 전액이 공제 대상(이 되는 것)이 아니에요」.
#: 문구 끝에서 부정어까지 조사·「되는 것」 정도만 허용하고, 문장 부호를 넘지 않는다 —
#: 「…공제 대상이에요. 아니, …」는 부정이 아니다.
...
def _corrects(answer: str, at: int, wrong: str) -> bool:
    ...
    정정으로 보는 꼴은 둘이다. ① 따옴표로 따오고 곁에 정정 표지가 있다. ② 문구 바로 뒤에
    부정이 붙는다(「~이 아니다」·「~되지 않는다」) — 이건 문구를 따온 것이 아니라 **그 반대를
    주장한 것**이라 오답이 아니다. ②가 없던 동안 「전환금 전액이 공제 대상이 되는 것이
    아니에요」가 오답 주장으로 잡혀 52번(송도윤)이 재작성까지 두 번 폐기되고 근거 원문으로
    떨어졌다(2026-09-22). 부정이 문구 **뒤에 바로** 붙을 때만이다 — 앞의 「아니」나 다른
    문장의 「않」은 세지 않는다(§6 「검증기가 옳은 문장을 거부하는 것은 …보다 나쁘다」는
    좁게 여는 것까지만 허용한다).
    """
    end = at + len(wrong)
    if _NEGATED.search(answer[end:end + _NEGATION_NEAR]):
        return True
