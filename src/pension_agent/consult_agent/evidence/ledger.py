...
def ledger_texts(evidence: list[Evidence]) -> list[str]:
    ...
    **표시(notices)도 허용 텍스트다.** 표시는 코드가 답변 아래에 세우는 문장인데(stale_mark 의
    「— <기준시점> 기준 표기입니다」, caution 비고, 팩트 상태), 그 안의 값(기준시점)은 형태
    요구가 본문에 녹여 쓰라고 하는 값이다. 그 문장이 `allow` 에 없던 동안 **시킨 대로 옮겨
    쓴 답변이 «자료 밖 날짜»로 통째로 버려졌다** — 채널 카드는 본문(_render_channel)에
    기준시점이 없고 표시에만 있어 채널 답변이 리허설 3턴 전부 근거 원문 덤프로 끝났다
    (2026-09-05 demo T3b · cases 2·3). 지워진 gap 31 과 같은 부류다(코드가 프롬프트에 실어
    보낸 것을 인용하면 폐기됐다). 표시는 전부 카드 데이터에서 코드가 만든 문장이라 넓혀도
    LLM 이 지어낼 자리가 없다. 2026-09-22 부터 표시는 <필수 인용> 으로 넘어가지 않는다 —
    그래도 이 허용은 남긴다. 시황·상품은 기준시점이 재료 본문에도 있어(`· 기준시점:`) 본문이
    그 값을 옮겨 쓰고, 코드가 아래에 세운 표시 문장 자체도 답변의 일부라 검사를 지나가야 한다.
    """
