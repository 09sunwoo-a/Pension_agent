...
#
# 항목이 있는 코드는 **caution 역할 문장을 가진 method 카드가 실제로 있는 요건**뿐이다
# (2026-09-16 실측 — caution 을 가진 method 카드는 7장: m.1-1·m.004·m.005·m.006·m.023·
# m.027·m.069). 그 전에는 다섯 코드만 있어서, 송도윤(dor·hlt·idl·nod·isa)처럼 «미운용
# 현금성자산»·«판매중단 펀드 보유»로 관리 대상이 된 고객에게 가드가 한 번도 안 붙었다 —
# m.1-1(사용계획 자금 먼저 걸러낼 것)·m.006(고유계정대라는 말을 고객에게 쓰지 말 것)·
# m.023(갈아탄 상품이 더 좋다고 약속하지 말 것)이 지식베이스에 있는데 코드가 요건과 잇지
# 않았다(§12 gap 35 「§8 가드도 붙지 않았다」). 요건 코드의 뜻은 `strategy_customer.CONDS`.
#
# 항목이 없는 코드(pen·isa·tax·add·nch…)는 재료가 없어서다 — 카드가 생기면 항목을 더한다.
# 말을 넓혀 억지로 걸리게 하지 않는다(루트 「지식베이스에 없는 기준은 만들지 않는다」).
TRIGGERS: dict[str, tuple[str, ...]] = {
    "low": ("수익률 하위", "수익률이 낮", "저조", "민감"),
    "dor": ("미접촉", "장기 미거래"),
    "dep": ("현금성", "고유계정대", "편중"),
    # 미운용 현금성자산 · 이탈위험 관찰(현금화 신호) — 둘 다 고유계정대가 큰 계좌이고, 재료는
    # 그 자금이 «사용계획 자금인지 방치 자금인지» 먼저 가르라는 방법론(m.1-1·m.006)이다.
    "idl": ("현금성", "고유계정대"),
    "out": ("현금성", "고유계정대"),
    # 판매중단·환매추천 펀드 보유 — m.023 「갈아탄 상품이 더 좋을 것이라고 약속하지 않는다」.
    "hlt": ("판매중단", "환매추천"),
    # 투자성향 불일치 — m.005 「반대 케이스는 원리금보장 100% 대상에서 제외(이중 접촉 방지)」.
    "mis": ("투자성향",),
    "nod": ("디폴트옵션 미",),
    "mat": ("만기",),
}
