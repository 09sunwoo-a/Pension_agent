...
도구 규약(Tool·ToolFailure)은 `tools/base.py` 에 있고, 도구 전체의 규약은 tools 패키지
머리말(`tools/__init__.py`)이 말한다. 이 파일이 `tools/` 가 아니라 여기 있는 이유는 원장
helper(`ledger.py`)가 Evidence 를 쓰기 때문이다 — evidence/ 는 tools/ 를 임포트하지 않는다
(`tests/infra/s03_boundaries.py`).
"""
