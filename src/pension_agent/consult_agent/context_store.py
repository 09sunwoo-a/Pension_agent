...
━━ 무엇을 · 얼마나 ━━
`history` 는 에이전트가 이미 최근 HISTORY_LIMIT(12)턴으로 자른 목록이고, 한 턴은 질문 문장 ·
슬롯 · pending 플래그 · 도구 이름에 **답변 원문**(ANSWER_KEEP 자 상한 — «줄여줘»가 다시 쓸
재료, state.Turn)이다. 답변이 보통 1~2KB 라 세션 하나가 길어야 수십 KB 이고, 상한 500 세션
이어도 십수 MB 안쪽이다.
