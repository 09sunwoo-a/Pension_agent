...
`answer` 안에서 **형태 판정과 답변 작성이 동시에 돈다**(`nodes/answer.py`). 둘은 서로의
...
판정값은 **넷**이다(§5 가 정의한 결론 넷). `answer` 는 그대로 내보내고, `ask` 는 써 둔
답을 버리고 되묻으며, `assume`(전제를 밝히고 답한다)·`none`(핵심 대상이 자료에 없다)은
작성 프롬프트에 블록을 얹어 **한 번 다시 쓴다**. 다시 쓰기가 비면 처음 답이 나간다 —
판정을 도우려던 장치가 답을 없애면 안 된다. 판정이 쓴 문장(전제·빠진 대상)에 원장 밖
수치가 있으면 그 등급은 버려진다(`clarify._quotable`): 그 문장은 작성 프롬프트로 들어가
되받아 적히는데, 원장 밖 수치를 되받으면 §6 이 답을 통째로 버리기 때문이다.

판정에는 갈래 재료(<근거>) 말고 블록 둘이 더 실린다. **«이미 정해진 것»**(`settled_block`)은
열려 있는 고객에 대해 코드가 원장에서 계산한 값이다 — 갈래를 만들지는 않지만 **정해 주는**
일은 한다(§5 · gap 30). 모르는 값은 싣지 않는다: 「미확인」이 그 블록에 있으면 판정이 그
축까지 정해진 것으로 읽는다. **«갈래»**(`state["branches"]`)는 적합성 게이트가 표시한 축이다 —
후보 목록 전체를 보는 곳이 거기뿐이라, 거기서 「애매하면 뺀다」로 한쪽을 떨어뜨리면 갈래가
있었다는 사실 자체가 사라진다. 갈래로 표시된 후보는 빼지 않고 남긴다. 둘 다 없으면 블록이
아예 안 붙는다 — 없는데 세우면 판정이 없는 갈래를 만든다.
...
**이번 턴에 무엇을 해 봤는지는 장부 하나에 쌓인다**(`state["steps"]` — 호출마다
`{tool, query, outcome}`, `outcome` 은 `found`·`miss`·`failed`). 근거를 못 낸 호출
(`miss`)은 다음 계획 프롬프트에 실린다(원장에는 성공한 재료만 쌓이므로, 이게 없으면 계획은
자기가 뭘 불러봤는지 모른 채 같은 호출을 반복한다). 죽은 호출(`failed`)은 프롬프트에
싣는 대신 그 도구를 이번 턴 카탈로그에서 뺀다 — 고장은 질의를 바꿔 낫지 않는다. 근거
...
- **카드 선택은 LLM 이 1차, n-gram 이 폴백이다.** 원래는 반대였는데, n-gram 은 문자 유사도라
  ...
  **LLM 이 살아서 0건을 냈을 때만** 돈다(LLM 장애는 폴백이 아니라 무응답이다, §11).
  `trigger_examples` 는 두 경로 모두의 입구다 — LLM 카드 목록 한 줄에는 앞 2개만 실리고
  (`kb._card_line`), n-gram 폴백은 전부와 제목을 함께 잰다(`kb.score_parts`). 2026-09-04 까지
  pitch 를 뺀 모든 종류가 첫 칸에 제목을 그대로 넣어 목록의 정보 칸이 하나뿐이었고,
  `fieldtip` 은 제목 하나뿐이었다. 지금은 변환기가 제목을 넣지 않고 본문 절로 채운다
  (`build_kb.triggers_of` · `CLAUDE.md` §3 「재료를 고르는 길」).
