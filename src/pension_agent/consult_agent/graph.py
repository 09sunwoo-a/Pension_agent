...
def employee_no(explicit: str | None, x_client_user: str | None) -> str | None:
    ...
    에서는 **앞 7자리**를 읽는다 — 그 값은 「사번 + uuid」꼴이고, 사번이 아닌 값도
    들어온다(`ask` 머리말 · `workb.as_emp_no`).
...
def ask(
    ...
    **사번 꼴일 때만** 쓴다(`workb.as_emp_no`) — 아래.
    ...
    ━━ x_client_user 는 「사번 + uuid」다 ━━
    게이트웨이가 넘기는 값은 사번 7자리 뒤에 LLM 중복 호출을 가르는 uuid 가 붙은 꼴이다
    (`3902172-550e8400-…`). 사번은 늘 맨 앞이므로 **앞 7자리**를 읽는다
    (`workb.as_emp_no`). 그 자리가 숫자 7자리가 아니면 사번으로 읽지 않는다 — 그 값은
    쿼터 버킷 이름이기도 해서 `pension-agent` 같은 값도 들어오기 때문이다.
    **명시한 `employee_id` 는 그대로 믿는다**(프론트가 «이것이 사번이다»라고 말한 값이다).
    둘 다 못 읽으면 환경변수 폴백으로 떨어지므로(`workb.employee_id`) 틀리는 방향이
    되돌릴 수 있는 쪽이다 — 없는 사번 앞으로 쪽지를 보내지 않는다.
    """
    global _AGENT
    if _AGENT is None:
        _AGENT = build_agent()
