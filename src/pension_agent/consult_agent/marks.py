...
같은 사고가 `customer_facing` 에서 한 번 더 났다. **주의(`notes_for`)는 선언이 거짓인
카드를 보는데, 재료를 조립하는 쪽(`facts_qa`·`procedure_qa`)은 참일 때만 표시를 붙이고
거짓은 침묵했다.** 그래서 답변 아래에는 "고객에게 그대로 안내하지는 마세요"가 서고, 본문은
바로 그 카드를 근거로 "고객에게 이렇게 안내하는 게 핵심"이라고 쓰는 일이 실제로 났다
(송도윤 S6 · fact.k04.f53 「퇴직금 5천만원 이상 + 비대면 계좌 — 수수료 면제」,
`customer_facing: false`). 작성 프롬프트에는 「'내부용'으로 표시된 자료는…」 규칙이 이미
있었지만, **그 «표시»가 재료에 없었으므로 지킬 수 없는 규칙이었다.** 재료에 싣는 표시도
`facing_note()` 로 여기서 만든다 — 같은 선언을 읽는 곳이 둘이면 한쪽만 고쳐진다.
"""
...
#: **재료**에 싣는 「고객에게 그대로 옮겨도 되는가」 표시 — 위 INTERNAL_NOTE 가 답변 아래에
#: 서는 주의라면 이것은 작성 LLM 이 보는 재료 안의 표시다. 둘은 같은 선언(`customer_facing`)을
#: 읽으므로 한 곳에서 만든다(모듈 docstring 「왜 한 곳인가」).
#:
#: 거짓 쪽 문구가 «금지»가 아니라 «옮기지 마세요»인 것은 의도적이다 — 내부용 자료도 답변
#: 재료로는 그대로 쓴다(§7). 옮기지 말라는 대상은 **고객에게 할 말**이다.
FACING_NOTE: dict[bool, str] = {
    True: "고객에게 그대로 안내 가능",
    False: "내부용 — 이 내용을 고객에게 할 말로 옮기지 마세요",
}


def facing_note(card: dict) -> str | None:
    """이 카드를 고객에게 그대로 옮겨도 되는지의 재료 표시. 선언이 없으면 None.

    선언은 참·거짓·없음 셋이고 **없음은 없음으로 둔다** — 거짓과 같이 취급하면 선언이 없는
    종류(pitch·method·segment)가 전부 내부용이 되고, 참과 같이 취급하면 확인되지 않은 자료가
    「고객에게 그대로 안내 가능」으로 선다. 둘 다 데이터에 없는 판단을 코드가 지어내는 것이다.
    """
    value = card.get("customer_facing")
    return FACING_NOTE[value] if isinstance(value, bool) else None
