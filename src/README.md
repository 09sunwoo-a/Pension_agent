...
| `HTTP 429` | 호출이 몰림 | 게이트가 **스스로 느려진다** — 429 마다 호출 간격이 3초 → 6 → 12 … 로 넓어지고(상한 30초) 성공하면 조금씩 돌아온다. 지금 간격은 `/health` 의 `rate_gate.interval_sec`. 상한에 닿았는데도 429 면 속도가 아니라 쿼터 문제이니 키·한도를 본다. 더 눌러야 하면 `.env` 에서 `LLM_MAX_CONCURRENCY=1` · `LLM_MIN_INTERVAL_SEC` 를 올린다. `prebuild_briefings` 는 한 고객이 429 로 끝나면 그 자리에서 멈춘다 — 잠시 뒤 다시 실행하면 저장된 고객은 건너뛴다 |
