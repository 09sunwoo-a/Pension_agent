...
| `HTTP 429` | 분당 호출 한도 | **`.env` 에 `LLM_CALLS_PER_MIN=10` 을 적는다**(행내 STG 한도 · 2026-09-08 확인). 코드가 60/10 = 6초 간격으로 환산해 처음부터 맞는 속도로 나간다 — 실측으로 429 가 3회 → 0회가 된다. 한도를 모르는 환경이면 적지 않아도 된다: 게이트가 429 마다 간격을 넓혀(3 → 6 → 12초, 상한 30초) 스스로 찾고 성공하면 돌아온다. 지금 상태는 `/health` 의 `rate_gate`. `prebuild_briefings` 는 한 고객이 429 로 끝나면 멈춘다 — 다시 실행하면 저장된 고객은 건너뛴다 |
| 한도 때문에 **너무 느림** | 브리핑 1건 = 12연쇄 | 분당 10회면 12명 선생성에 **최소 14분**이다(144회 ÷ 10). 재시도를 조여도 줄지 않는다 — 줄이려면 버킷을 나누거나(`LLM_CLIENT_USER_SPREAD`, `.env` 주석의 ⚠ 확인) 보내는 호출을 줄여야 한다 |
