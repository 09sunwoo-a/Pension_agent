...
- **로그인 사번은 호출이 넘겨준다.** `input_value` 의 `employee_id`, 없으면
  `x_client_user` 를 **사번 꼴(숫자 7자리)일 때만** 읽는다 — 그 값은 LLM 쿼터 버킷
  이름이라 사번이라는 보장이 없다(`pension-agent` 같은 값이 온다). 진입점 → 상태 →
  발송이 같은 값을 보고, 그 값이 **누구 이름으로 나가나**(MCP 인증·행내 감사 기록)도
  정한다. 요청 로그의 `emp_no=` 로 확인한다 — `-` 면 환경변수 폴백이다.
