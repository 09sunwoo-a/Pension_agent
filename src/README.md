...
- **시간축이 둘이다.** `customer.AS_OF`(원장 스냅샷이 찍힌 날 — 잔액·수익률의 시점)와
  ...
  **고정은 회귀 모듈이 스스로 건다**(`tests.pin_today`) — 임포트로는 걸리지 않으므로
  리허설·디버그 CLI·Streamlit 은 실제 날짜로 돈다. 리허설은 실행 머리에 오늘·원장
  기준일을 찍는다. **로스터는 날이 바뀌면 다시 만들어진다**(`customer.refresh_roster`) —
  만기 잔여일수·미접촉 경과일은 Profile 을 만들 때 계산되므로, 자정을 넘긴 프로세스에서
  그 값들만 어제 것으로 남지 않게 한다.
