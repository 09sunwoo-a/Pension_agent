...
(refs/dockerfile.md). 디렉터리로는 `briefing_cache/` 가 하나 더 있는데 이쪽은 저장소에
있으므로(커밋한다) 따로 만들 것이 없다 — 지우고 빌드하면 그때는 실패한다
(아래 「미리 만들어 두기」).
...
git add briefing_cache && git commit          # 3. 커밋 전에 읽는다 — 아래
docker build -f Dockerfile -t pension-agent .
```

**`briefing_cache/` 는 커밋한다.** 예전에는 막았고 그 이유는 지금도 유효하다 — 브리핑은
LLM 이 쓴 산문이라 저장소에 넣으면 사람이 검토하지 않은 문장이 자산으로 굳는다. 그보다
앞서는 사정(위: 미리 만들어 두지 않으면 배포본이 답하지 못한다)이 있어 뒤집었으므로,
**커밋 전에 읽는다.** 그 파일이 곧 화면에 뜨는 문장이다. 코드·데이터를 고쳤으면 지문이
어긋나 읽히지 않으니 다시 만들어 다시 커밋한다.

키 없이 돌린 실행의 브리핑은 애초에 저장되지 않는다(`agent.propose()`) — LLM 섹션이
통째로 빈 파일이 커밋되면 «출처는 진짜인데 내용이 빈» 브리핑이 배포된다.
