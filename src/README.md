...
flowchart TB
    ...
    subgraph PITCH["pitch_agent — 화법 코칭 (LangGraph)"]
...
- **저작**: 원본 문서를 LLM이 읽고 `kinds.json`에 선언된 종류(kind)의 JSON으로 뽑아낸 뒤,
  ...
  통과해야 활성화된다.
- **pitch_agent**는 결정론적 검색(n-gram+태그)을 우선 쓰고, 그마저 0건일 때만 이미 저작된
  카드 목록으로 검색 범위를 한정한 LLM 재랭킹(`llm_rerank`)이 보조한다 — 저장소 전체가 아니라
  검증을 통과한 카드만 후보가 되므로, 미승인 내용이 답변에 섞일 수 없다.
- **strategy_agent**는 pitch_agent의 데이터를 실시간 검색하지 않는다. 전략마다 어떤 화법
