...
(검토 게이트 — `common/kb_base.py::iter_knowledge_files`). 작업 중인 초안이나 보관본을 데이터
...
원본이 사내 게시글(작성자 실명·부점·직급이 붙는 자료)인 경우, 저작 결과에는 그 개인 식별정보를
포함하지 않는다(§4). `common.schema validate` 실행 시 `[개인정보의심]` WARN 이 뜨면 해당
레코드를 열어 실제로 개인정보인지 확인하고 제거한다.
